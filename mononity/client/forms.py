...
class RegistrationForm(forms.Form):
    ...
    bankname = forms.CharField(label=u'입금은행', min_length=1, max_length=100)
    accnum = forms.IntegerField(label=u'계좌번호')
...
class AppRegForm(forms.Form):
    apikey = forms.CharField(label=u'앱apikey', max_length=32)
    appname = forms.CharField(label=u'앱이름',min_length=4, max_length=30)
    def clean_appname(self):
        username = self.cleaned_data['appname']
        if len(username)>30:
            raise forms.ValidationError('앱 이름이 너무 깁니다. 줄여주세요')
        if re.search(r'^[A-Za-z0-9_-]*$', username):
            pass
        else:
            raise forms.ValidationError('앱이름에 잘못된 문자가 들어가 있습니다. 한글 불가 합니다.')
